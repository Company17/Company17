...
Branch rules:
- `main` is always deployable; Vercel production tracks it.
...
Accounts needed before the first task: Neon (Postgres), Vercel (import the repo), Resend (email for login links). Put the keys in Vercel → Project → Environment Variables and in `.env.local`.
...
| T06 | PDF upload → page images | `app/actions/upload.ts`, `app/api/render/route.ts`, Vercel Blob | A 40-page PDF becomes 40 revisions with stored width/height in under 5 min |
...
| T09 | Server-side diff | `app/api/diff/route.ts`, `lib/diff.ts` (sharp) | Suggestions returned for a revision pair; cached; title-block region excluded; translation offset applied |
...
| T13 | COR PDF export | `lib/pdf/cor.tsx` (`@react-pdf/renderer`), `lib/crop.ts` (sharp) | PDF has cover, items, totals, one page per cloud with the sheet crop and the cloud drawn on it |
...
2. Import the repo into Vercel; confirm the preview URL opens and the review tool works.
3. Create the Neon project and Resend account; add keys to Vercel and `.env.local`.
